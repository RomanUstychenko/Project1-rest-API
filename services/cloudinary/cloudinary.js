...
exports.createImageUrl = (publicId, w, h) => {
  return cloudinary.url(publicId, {
    transformation: [{ width: w, height: h, gravity: "auto", crop: "fill" }],
    secure: true,
  });
};
...
exports.deleteImagesSection = async (req, res) => {
  ...
  cloudinary.api.delete_resources(publicIds, (error, result) => {
    ...
    const notFoundImages = Object.keys(deletedImages).filter(
      (id) => deletedImages[id] === "not_found"
    );
    ...
    if (notFoundImages.length > 0) {
      ...
      return res
        .status(404)
        .json({ message: "Some images not found", notFoundImages });
    }

    res
      .status(200)
      .json({ message: "All images deleted successfully", result });
  });
};
